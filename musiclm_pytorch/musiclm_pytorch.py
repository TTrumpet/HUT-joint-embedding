...
class AudioSpectrogramTransformer(nn.Module):
    def __init__(
        ...
        spec_n_fft = 128,
        spec_power = None, # changed from 2 (float32 type)
        spec_win_length = 24,
...
class MuLaNEmbedQuantizer(nn.Module):
    ...
    def __init__(
        ...
        cond_embeddings = nn.Parameter(torch.randn(rq_num_quantizers, codebook_size, conditioning_dims))
        nn.init.normal_(cond_embeddings, std = 0.02)

        self.cond_embeddings[namespaces] = cond_embeddings

        self.set_default_namespace(namespaces)
